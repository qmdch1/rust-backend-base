...
| 단계 | 스크립트 | 실행 빈도 | 역할 |
|------|----------|-----------|------|
| 1 | [`deploy.sh`](deploy.sh) | 서버 초기 세팅 시 **1회** | Docker, k3s, NGINX Ingress 설치 |
| 2 | [`scripts/app-deploy.sh`](scripts/app-deploy.sh) | 코드 변경 시 **반복** | 이미지 빌드 → .env → K8s Secret → Kustomize 배포 |
