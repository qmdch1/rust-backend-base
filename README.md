...
**Jenkins 서버**
- `.env` (Credentials에 Secret file로 등록)
- Docker
- kubectl + kubeconfig (Prod 서버 연결용)

**Prod 서버**
- `deploy.sh` 실행 (1회) → Docker, Registry, k3s, Ingress 설치

### 배포 흐름

```
[로컬] Git Push → [Jenkins] 빌드/테스트/이미지 push → [Prod] K8s 배포
```

| 파일 | 역할 | 실행 빈도 |
|------|------|-----------|
| [`deploy.sh`](deploy.sh) | Prod 서버 인프라 설치 (Docker, Registry, k3s, Ingress) | 서버 초기 세팅 시 **1회** |
