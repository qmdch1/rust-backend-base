...
| 단계 | 설치 항목 | 설명 |
|------|-----------|------|
| 1 | 시스템 패키지 | curl, git, openssl 등 기본 도구 |
| 2 | Docker CE | 컨테이너 이미지 빌드용 |
| 3 | k3s | 경량 Kubernetes (kubectl 포함) |
| 4 | NGINX Ingress | 외부 트래픽 라우팅 + TLS |
| 5 | 앱 빌드 & 배포 | Docker 이미지 빌드 → k3s import → Kustomize 배포 |

```bash
# 기본 (prod 환경, 시크릿 자동 생성)
sudo ./deploy.sh

# 환경 지정
sudo DEPLOY_ENV=dev ./deploy.sh       # dev | staging | prod

# 시크릿·도메인 직접 지정
sudo DB_PASSWORD="mypass" JWT_SECRET="mysecret" DOMAIN="api.example.com" ./deploy.sh
```

<details>
<summary><b>deploy.sh 전체 스크립트 보기</b></summary>
...
│   │   ├── configmap.yaml
│   │   ├── secret.yaml
│   │   ├── deployment.yaml     # App (replicas, probes, resources)
...
k8s/                → 실제 배포 (Kustomize 기반 환경 분리)
  base/             → 공통 매니페스트
  overlays/
    dev/            → 1 replica, debug log, 낮은 리소스
    staging/        → 2 replicas, info log
...
```bash
# 1. Docker 이미지 빌드 & 푸시
docker build -t your-registry/rust-backend:v1.0.0 .
docker push your-registry/rust-backend:v1.0.0

# 2. 이미지 태그 반영 (deployment.yaml의 image 수정)
# 또는 kustomize set image 사용:
cd k8s/overlays/prod
kustomize edit set image rust-backend=your-registry/rust-backend:v1.0.0
...
kubectl apply -k k8s/overlays/prod     # 프로덕션
```
...
Base의 `secret.yaml`은 **템플릿**입니다. 프로덕션에서는 다음 중 하나를 사용하세요:

```bash
# 방법 1: kubectl로 직접 생성
kubectl create secret generic rust-backend-secret \
  --namespace=rust-backend-prod \
...
# 방법 3: External Secrets Operator (AWS/GCP/Vault 연동)
```
